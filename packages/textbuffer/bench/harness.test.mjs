...
import { applyOracle, indexLines, makeFixtures, normalizeInput, oraclePoint, oracleQuery, safeBoundary } from './fixtures.mjs'
...
for (const name of ['singapore', 'vscode']) describe(name + ' adapter contract', () => {
  let factory
  beforeAll(async () => { factory = await loadAdapter(name) })

  it.each(['', '\n', 'abc', 'abc\n', '\n\n', 'שלום 😀e\u0301\t中\nlast', '\ufeffa\r\nb\rc\u2028d'])('agrees with normalized string semantics for %j', (input) => {
    const expected = normalizeInput(input)
    assertBuffer(factory.create(expected), expected)
  })

  it.each(fixtures.filter((fixture) => fixture.mode === 'edit'))('matches every intermediate edit in $name', (fixture) => {
  ...
  it.each(fixtures.filter((fixture) => fixture.mode === 'query'))('checks exact results and consumption for $name', (fixture) => {
    const buffer = factory.create(fixture.initial)
  ...
  it('rejects broken reads even when a supplied checksum looks correct', () => {
    ...
    expect(() => validate(factory, fixture, { buffer, checksum: fixture.expectedDigest })).toThrow()
  })
...
describe('reproducibility and reporting', () => {
  it('replays the same fixture identities for a seed and changes edit traces for another', () => {
    ...
    expect(sha256(JSON.stringify(makeFixtures('smoke', 43)))).not.toBe(sha256(JSON.stringify(fixtures)))
  })
  ...
  it('reports nearest-rank p95 and the correct even-sample median', () => {
    expect(statistics([4, 1, 3, 2])).toEqual({ count: 4, min: 1, median: 2.5, p95: 4, max: 4, mean: 2.5 })
    expect(statistics(Array.from({ length: 20 }, (_, index) => index + 1)).p95).toBe(19)
    expect(() => statistics([])).toThrow()
    expect(() => statistics([NaN])).toThrow()
  })

  it.each([['--samples', '0'], ['--samples', 'NaN'], ['--warmups', '-1'], ['--profile', 'unknown'], ['--seed'], ['--oops', '1']])('rejects malformed options %j', (...args) => {
