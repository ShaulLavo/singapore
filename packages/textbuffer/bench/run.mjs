...
export function optionsFrom(args) {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    if (!['profile', 'samples', 'warmups', 'seed', 'output', 'only'].includes(key)) throw new Error(`Unknown option ${args[index]}`)
  ...
  for (const [key, low, high] of [['samples', 1, 99], ['warmups', 0, 20], ['seed', 0, 0xffffffff]]) {
    if (!Number.isSafeInteger(options[key]) || options[key] < low || options[key] > high) throw new Error(`Invalid ${key}`)
  }
  return options
}

function git(args) {
  try { return execFileSync('git', ['-C', packageRoot].concat(args), { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim() }
  catch { return null }
}
...
export function markdown(report) {
  ...
  const format = (value) => value ? `${value.median.toFixed(3)} / ${value.p95.toFixed(3)}` : 'not equivalent'
  for (const row of report.workloads) {
    lines.push(`| ${row.name} | ${row.operations} | ${format(row.engines.singapore?.timeMs)} | ${format(row.engines.vscode?.timeMs)} | ${row.ratio === null ? 'separate lane' : row.ratio.toFixed(2) + 'x'} |`)
  }
  lines.push('', '## Post-GC retained memory', '',
  ...
  for (const row of report.workloads) for (const [engine, data] of Object.entries(row.engines)) {
    lines.push(`| ${row.name} | ${engine} | ${Math.round(data.retainedBytes.heapUsed.median)} | ${Math.round(data.retainedBytes.external.median)} | ${Math.round(data.retainedBytes.arrayBuffers.median)} |`)
  }
  lines.push('', '## Interpretation', '',
    ...
    report.options.profile === 'smoke' ? '- Smoke results validate the harness only; do not use these tiny samples to rank engines.' : '- Synthetic workload results on one machine require repetition across seeds, machines and browser engines.', '')
  return lines.join('\n')
}
...
async function main() {
  ...
  const report = {
    schemaVersion: 1, createdAt: new Date().toISOString(), options,
    environment: {
      ...
      totalMemory: os.totalmem(), workerFlags: ['--expose-gc'],
    },
    source: { commit: git(['rev-parse', 'HEAD']), dirty: git(['status', '--porcelain', '--', '.']), identity },
    upstream, workloads: [],
  }
  try {
    for (const [workloadIndex, fixture] of fixtures.entries()) {
      ...
      for (let sample = 0; sample < options.samples; sample += 1) {
        let engines = (sample + workloadIndex) % 2 ? ['vscode', 'singapore'] : ['singapore', 'vscode']
        if (fixture.category === 'singapore-only') engines = ['singapore']
        for (const engine of engines) {
          const output = execFileSync(process.execPath, ['--expose-gc', path.join(benchRoot, 'worker.mjs'), engine, filename, hash, String(options.warmups)], {
            cwd: packageRoot, encoding: 'utf8', timeout: 120000, maxBuffer: 8 * 1024 * 1024,
          })
      ...
      const engines = {}
      for (const [name, values] of Object.entries(samples)) if (values.length) {
        engines[name] = {
          timeMs: statistics(values.map((value) => value.elapsedMs)),
          retainedBytes: Object.fromEntries(['heapUsed', 'external', 'arrayBuffers', 'rss'].map((key) => [key, statistics(values.map((value) => value.retainedBytes[key]))])),
          samples: values,
        }
      }
      if (engines.vscode) assert.equal(samples.singapore[0].checksum, samples.vscode[0].checksum, 'shared result checksum')
      const row = {
        name: fixture.name, category: fixture.category, fixtureSha256: hash,
        inputCodeUnits: fixture.initial.length, inputUtf8Bytes: Buffer.byteLength(fixture.initial),
        operations: fixture.operations.length || 1, setupOperations: fixture.setup.length,
        primitiveEdits: fixture.operations.reduce((total, operation) => total + (operation.kind === 'batch' ? operation.edits.length : operation.kind === 'edit' ? 1 : 0), 0),
        engines, ratio: engines.vscode ? engines.singapore.timeMs.median / engines.vscode.timeMs.median : null,
      }
      report.workloads.push(row)
      console.log(`${row.name}: Singapore ${engines.singapore.timeMs.median.toFixed(3)} ms${engines.vscode ? `; VS Code ${engines.vscode.timeMs.median.toFixed(3)} ms; ratio ${row.ratio.toFixed(2)}x` : '; separate capability lane'}`)
    }
    ...
    report.checksum = report.workloads.reduce((sum, row) => consume(row.engines.singapore.samples[0].checksum, sum), 2166136261)
    const output = path.resolve(options.output ?? path.join(benchRoot, 'results', options.profile + '.json'))
