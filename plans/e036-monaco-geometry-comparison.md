...
Raw results are in `/work/tmp/editor-e036/` (`step1-b.json`, `fonts.json`, `blink-headless.json`, `blink-headed.json`, `fonts-after.json`,
`layout-trace*`).
...
| 2. Monospace verification            | Hit test at every third column's DOM x: monospace, JetBrains Mono NF (ligatures on) and iA Writer Mono S: 0 of 339 wrong. Liberation Sans and Noto Sans: 339 of 339 wrong, up to 37 and 39 columns off.                                                                                                                                                                                                                                        | Go, implemented (see below). Platform's `editor.fontFamily` accepts any font.                                                                                                 |
...
| 5. Caret blink                       | Idle 20 s, 3 interleaved rounds, whole-browser CPU. Headed on Wayland with the GPU process on `/dev/dri/renderD128`: CSS `steps()` 1.08%, JS interval 0.85%, no blink 0.38% of one core; the GPU process takes 0.11–0.13 s vs 0.08 s per 20 s. Headless gave the same order (0.73/0.48/0.23%).                                                                                                                                                 | No-go. The interval saves 0.23 points of a core. The Chromium issue Monaco cites, frames at display rate for a 2 Hz blink, does not reproduce: it would cost many times this. |
...
writes the caret window through `updateText`/`updateSelection`. A `textupdate` outside a
composition becomes `textUpdateEdit`, a rewrite of the written window said around the caret, and
goes through the existing multi-caret path. During a composition it tracks the candidate's span,
draws it with the existing preedit, and commits one edit at `compositionend`. `characterboundsupdate`
answers with the preedit's glyph rects. Typed characters still go through `beforeinput`, which
keeps auto-close and snippets, because preventing it withholds the update.
`test/editContextInput.browser.test.ts` runs every case on both routes through CDP IME commands.
The textarea route fails one: a composition that replaces a range behind the caret (the shape of
