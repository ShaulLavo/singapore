...
- `activate` runs once per editor for the same plugin object. State belongs in `activate` or in a
  contribution, never in the factory closure. `createBracketMatchPlugin`
  ([bracketMatchPlugin.ts:44](../../packages/editor/src/bracketMatchPlugin.ts)) and
  `createMergeConflictPlugin` ([mergeConflictPlugin.ts:118](../../packages/editor/src/mergeConflictPlugin.ts))
  break this: their `controller` is reassigned by each editor that activates the same object.
- A registration made inside `install` or `activate` is unwound with that phase. One made later, from
  a promise or timer, is owned by the host and unwound only when the editor is disposed
  ([plugins.ts ownRegistration](../../packages/editor/src/plugins.ts)). A command registered that way
  still answers after `setPlugins([])`.
- A contribution's own registrations are unwound by the editor only if its factory throws or returns
  null ([Editor.ts createContributionSafely](../../packages/editor/src/editor/Editor.ts)). After a
  successful create, the contribution's `dispose` is the only thing that releases them. A keymap
  context key registered by a contribution whose `dispose` forgets it still reads true after
  `setPlugins([])`, and `onDidType` listeners are never claimed at all.
...
Status values: **public** (exported and used), **internal** (exists, not exported), **host** (the
host's API, not a plugin hook), **proposed**, **unsupported** (deliberately absent).

### Document edits

| Hook                                                    | Scope, phase, rights                                                                 | Order, cancel, dispose                                       | Consumer            | Status |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------ | ------------------- | ------ |
| `EditorEditContributionContext.applyEdits`              | Per editor. Any time. One batch against the current snapshot, one undo entry.        | Rejected when not editable. One optional result selection.   | find, lsp-plugin    | public |
| `startSnippetSession`                                   | Per editor, edit context.                                                            | Ends on the snippet's own exit rules.                        | lsp-plugin          | public |
| `materializeFullText`, `getTextSnapshot`                | Per editor, edit and decoration contexts. `TextReadSnapshot` reads rows and windows. | Full text is O(n) and gated by `check:full-text`.            | diff                | public |
| `getDocumentSyncPoint`, `changesSinceDocumentSyncPoint` | Per editor. Recovers every edit since a point.                                       | Returns null across a gap or segment change.                 | diff, lsp           | public |
...
| Hook                                               | Scope, phase, rights                                   | Order, cancel, dispose                             | Consumer                   | Status |
| -------------------------------------------------- | ------------------------------------------------------ | -------------------------------------------------- | -------------------------- | ------ |
| `getSelections`                                    | Edit and internal feature contexts. Resolved offsets.  | Current at call time.                              | find, lsp-plugin           | public |
| `setSelection`, `setSelections`                    | View context. Reveal options.                          | Runs in an editor operation.                       | find, lsp, diff            | public |
| `snapshot.selections` with update kind `selection` | View contributions.                                    | Every view contribution is visited for every kind. | bracket match, occurrences | public |
| `EditorSelectionRangeProvider`                     | Per editor. Asked on every expand; union of providers. | No invalidation event: read fresh.                 | tree-sitter                | public |
| `trackRanges`, `trackPoint`                        | View context. Anchors that follow edits.               | Live until dropped; no disposal needed.            | find                       | public |
...
| Hook                       | Scope, phase, rights                                                                | Order, cancel, dispose                                                    | Consumer                         | Status      |
| -------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | -------------------------------- | ----------- |
| `registerPressParticipant` | View context. Asked before a press becomes a caret; `true` claims it (E050 row 5).  | Registration order; first claim wins; the editor prevents the default.    | diff, definition links           | public      |
| `registerNonCaretRows`     | View context. Caret moves step over the rows.                                       | Claimed during creation only.                                             | diff                             | public      |
| `registerKeymapContextKey` | View context. Read when a key is matched (E050 row 6).                              | Unregistered keys read false.                                             | find, completion, signature help | public      |
| `onDidType`                | View context. After the typed edit lands.                                           | Not claimed by the editor; the contribution must dispose it.              | auto-close consumers             | public      |
| `EDITOR_PASTE_HANDLER`     | Language feature token. First handler that answers takes the paste.                 | Selector score, then priority, then registration order.                   | built-in paste handlers          | public      |
...
| Hook                                                                   | Scope, phase, rights                                                      | Order, cancel, dispose                                               | Consumer                       | Status   |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------- | ------------------------------ | -------- |
| `registerSyntaxProvider`                                               | Per editor. Creates a session per document.                               | First provider returning a session wins; change reloads the session. | tree-sitter                    | public   |
| `registerHighlighter`                                                  | Per editor. Session per document; `initialHighlightStatus` (E050 row 11). | First provider returning a session wins.                             | shiki                          | public   |
| `createEditorLanguageFeatureToken`, `registerProvider`, `getProviders` | Library-defined, many providers.                                          | Language match, then priority, then registration order.              | lsp-plugin, hover              | public   |
| `createEditorCapabilityToken`, `registerFeature`, `getFeature`         | Library-defined, one owner per editor.                                    | A second owner throws. No change subscription.                       | find, minimap, merge conflicts | public   |
...
| Hook                                | Scope, phase, rights                                             | Order, cancel, dispose                                             | Consumer                 | Status   |
| ----------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------ | ------------------------ | -------- |
| `EditorInjectedTextRowProvider`     | Per editor. Rows derived from a `TextReadSnapshot`.              | `onDidChangeInjectedTextRows` invalidates.                         | diff, merge conflicts    | public   |
| `EditorInlineReplacementProvider`   | Per editor. Called when the parse settles; providers compose.    | Outermost span first.                                              | markdown                 | public   |
| `EditorGutterContribution`          | Per editor. Cells created, updated, disposed by the gutter.      | Unique `id`; `interactive` cells take pointer events (E050 row 9). | line, fold, diff gutters | public   |
| `getRowPresentation`                | View context. Element handle with an abort signal (E050 row 10). | Aborts before text replacement, recycling and view disposal.       | decode                   | public   |
...
| Hook                                                 | Scope, phase, rights                                                    | Order, cancel, dispose                                                | Consumer            | Status |
| ---------------------------------------------------- | ----------------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------- | ------ |
| `EditorDecorationContribution` + `decorations` store | Per editor. Entries follow edits; `handleEditorChange(change \| null)`. | Entries owned per source; a throwing update removes the contribution. | diff                | public |
| `setRowDecorations`, `clearRowDecorations`           | Decoration context. Keyed by `sourceId`.                                | A second owner of a source ID is rejected.                            | diff                | public |
| `setRangeHighlight`, `clearRangeHighlight`           | View context. CSS highlight by name.                                    | Names are not owned; prefix with `highlightPrefix`.                   | bracket match, find | public |

### Frame work

| Hook                                 | Scope, phase, rights                                                     | Order, cancel, dispose                                 | Consumer               | Status      |
| ------------------------------------ | ------------------------------------------------------------------------ | ------------------------------------------------------ | ---------------------- | ----------- |
| `EditorViewContribution.update`      | One snapshot per pass; measure first, then write.                        | Reentrant requests queue, depth capped at 32.          | all view contributions | public      |
| `requestViewUpdate`                  | View context. Schedules a `layout` pass.                                 | Coalesced into the queue.                              | minimap, sticky scroll | public      |
| `captureVisiblePaint`, `snapshotKey` | Paint restoration.                                                       | A pending capture at restore removes the contribution. | scope lines            | public      |
...
| Hook                                                     | Scope, phase, rights                                  | Order, cancel, dispose                         | Consumer               | Status |
| -------------------------------------------------------- | ----------------------------------------------------- | ---------------------------------------------- | ---------------------- | ------ |
| `updateViewport`                                         | View contribution. Display rate, no snapshot rebuild. | Only contributions that define it are visited. | minimap, sticky scroll | public |
| `setScrollPosition`, `revealLine`                        | View context. Updates both axes (E050 row 8).         | —                                              | find, diff             | public |
| `reserveOverlayWidth`, `onDidChangeReservedOverlayWidth` | View context (E050 row 7).                            | Never coalesced.                               | find, minimap          | public |
| `registerWheelScrollTarget`                              | External overlay joins native scrolling.              | Dispose with the contribution.                 | find widget            | public |
...
| Hook                                                 | Scope, phase, rights                                 | Order, cancel, dispose                                      | Consumer                | Status   |
| ---------------------------------------------------- | ---------------------------------------------------- | ----------------------------------------------------------- | ----------------------- | -------- |
| Update kinds `document` and `clear`                  | View contributions, after the new document renders.  | No will-change phase; row handles abort before replacement. | every view contribution | public   |
| `handleEditorChange(null)`                           | Decoration and feature contributions on replacement. | —                                                           | diff, merge conflicts   | public   |
...
| Hook                                                                  | Scope, phase, rights                                           | Order, cancel, dispose                                                   | Consumer         | Status   |
| --------------------------------------------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------ | ---------------- | -------- |
| `EditorPlugin.install`, `activate`, `update`, `deactivate`, `dispose` | Per editor.                                                    | Phase registrations unwind with the phase; a throw unwinds partial work. | every plugin     | public   |
| Contribution providers (view, command, capability, edit, decoration)  | Created per editor, late providers get an initial update.      | A throwing factory or update removes the contribution and logs it.       | every plugin     | public   |
...
| `registerAmbientEditorPlugin`                                         | Module-global demand-loaded plugin.                            | Installed while a provider of `demand` exists.                           | hover            | public   |
| `registerLogger`, `log`                                               | Per editor.                                                    | A throwing logger is ignored.                                            | Platform logging | public   |
